...
async function syncAdditionalPricingModels() {
  console.log('\n💰 Syncing Additional Pricing Models (from Discount Rules)\n');
  console.log('='.repeat(70) + '\n');

  const db = getDatabase();
  const syncId = db.prepare(`
    INSERT INTO sync_log (entity_type, sync_started_at, status)
    VALUES ('pricing_models_additional', CURRENT_TIMESTAMP, 'running')
  `).run().lastInsertRowid;

  const stats = { fetched: 0, inserted: 0, updated: 0, status: 'success', error: null };

  try {
    // Extract pricing model IDs from discount rules
    const discounts = db.prepare('SELECT rules FROM standard_discounts').all();
    const pricingModelIds = new Set();

    discounts.forEach(discount => {
      const rules = JSON.parse(discount.rules);
      rules.forEach(rule => {
        if (rule.pricingModelId) {
          pricingModelIds.add(rule.pricingModelId);
        }
      });
    });

    const uniqueIds = Array.from(pricingModelIds);
    console.log(`📋 Found ${uniqueIds.length} unique pricing model ID(s) in discount rules\n`);
    ...
    console.log('\n' + '='.repeat(70));
    console.log('\n✅ Additional pricing models synced successfully!\n');
