...
async function syncInventoryModels() {
  ...
  try {
    // Use the best search term found
    const searchTerm = 'e ';

    // First, get all IDs from search
    ...
    console.log(`📋 Found ${totalElements} inventory models across ${totalPages} pages\n`);

    const allIds = new Set();

    // Collect all IDs
    console.log('📥 Fetching all inventory model IDs...\n');
    for (let page = 0; page < totalPages; page++) {
      try {
        ...
        process.stdout.write(`\r   Progress: ${progress}% (${allIds.size} unique IDs collected)`);
    ...
    console.log(`\n\n✅ Collected ${allIds.size} unique inventory model IDs\n`);
    console.log('='.repeat(70));
    console.log('\n📥 Fetching full details for each model...\n');

    const idsArray = Array.from(allIds);

    for (let i = 0; i < idsArray.length; i++) {
      const modelId = idsArray[i];
      ...
      try {
        ...
        // Progress indicator
        if ((i + 1) % 10 === 0 || i === idsArray.length - 1) {
          const progress = ((i + 1) / idsArray.length * 100).toFixed(1);
          process.stdout.write(`\r   Progress: ${progress}% (${i + 1}/${idsArray.length}) - ${stats.inserted} inserted, ${stats.updated} updated, ${stats.errors} errors`);
        }
    ...
    console.log('\n' + '='.repeat(70));
    console.log('\n✅ Inventory models synced successfully!\n');
